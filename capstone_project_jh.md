...
An additional challenge was implementing the regression model. Due to consistently poor results, this was abandoned.
...
One interesting result is the running a convolutional neural network on the unbalanced dataset, as seen in the pubchem_conv [report](pubchem_conv.html) and [notebook](pubchem_conv.ipynb), provided good accuracy in classifying false compounds, as can be seen by the following classification report:
